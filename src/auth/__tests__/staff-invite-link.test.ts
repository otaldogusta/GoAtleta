...
describe("staff invite entry", () => {
  it("accepts only email authentication proof, not a bare invite or recovery", () => {
    ...
    expect(
      parseStaffInviteFragment(
        `#code=TEST-CODE&token_hash=${hash}&type=magiclink`,
      ),
    ).toEqual({ code: "TEST-CODE", token_hash: hash, type: "magiclink" });
    expect(parseStaffInviteFragment("#code=TEST-CODE")).toBeNull();
    expect(
      parseStaffInviteFragment(
        `#code=TEST-CODE&token_hash=${hash}&type=recovery`,
      ),
    ).toBeNull();
  });
  it("does not approve an invitation merely because a role already exists", () => {
    ...
    expect(pending).toMatch(
      /await claimTrainerInvite\(code\);\s+setAccessApproved\(true\);/,
    );
  });
});
