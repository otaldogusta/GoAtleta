...
export const db = {
  ...
  async getFirstAsync<T>(sql: string, params: unknown[] = []) {
    ...
    // Query for monthly_planning_blueprint by classId, year and month.
    if (normalized.startsWith("select * from monthly_planning_blueprints")) {
      ...
      const month = Number(params[2] ?? 0);
      const cycleId = String(params[3] ?? "");
      const found = monthlyPlanningBlueprints.find(
        (bp) =>
          bp.classId === classId &&
          bp.year === year &&
          bp.month === month &&
          (!cycleId || bp.cycleId === cycleId)
      );
      return (found ?? null) as T;
    }
  ...
  async runAsync(sql: string, params: unknown[] = []) {
    ...
    if (normalized.startsWith("insert or replace into daily_lesson_plans")) {
      const plan = {
        ...
        sessionComponentsJson: String(params[7] ?? "[]"),
        sessionEnvironment: String(params[8] ?? ""),
        sessionPrimaryComponent: String(params[9] ?? ""),
        warmup: String(params[10] ?? ""),
        mainPart: String(params[11] ?? ""),
        cooldown: String(params[12] ?? ""),
        observations: String(params[13] ?? ""),
        generationVersion: Number(params[14] ?? 1),
        derivedFromWeeklyVersion: Number(params[15] ?? 1),
        generationModelVersion: String(params[16] ?? ""),
        generationContextSnapshotJson: String(params[17] ?? "{}"),
        syncStatus: String(params[18] ?? "in_sync"),
        outOfSyncReasonsJson: String(params[19] ?? "[]"),
        manualOverridesJson: String(params[20] ?? "{}"),
        manualOverrideMaskJson: String(params[21] ?? "[]"),
        lastAutoGeneratedAt: String(params[22] ?? ""),
        lastManualEditedAt: String(params[23] ?? ""),
        createdAt: String(params[24] ?? ""),
        updatedAt: String(params[25] ?? ""),
      };
    ...
    if (normalized.startsWith("insert or replace into monthly_planning_blueprints")) {
      const blueprint = {
        ...
        month: Number(params[4] ?? 0),
        title: String(params[5] ?? ""),
        macroIntent: String(params[6] ?? ""),
        pedagogicalProgression: String(params[7] ?? "[]"),
        weeklyFocusDistributionJson: String(params[8] ?? "{}"),
        constraintsJson: String(params[9] ?? "{}"),
        contextSnapshotJson: String(params[10] ?? "{}"),
        generationModelVersion: String(params[11] ?? ""),
        generationVersion: Number(params[12] ?? 1),
        syncStatus: String(params[13] ?? "in_sync"),
        lastAutoGeneratedAt: String(params[14] ?? ""),
        lastManualEditedAt: String(params[15] ?? ""),
    ...
    if (normalized.startsWith("insert or replace into planning_cycles")) {
      const cycle = {
        ...
        periodizationPolicyJson: String(params[8] ?? ""),
        policyVersion: Number(params[9] ?? 1),
