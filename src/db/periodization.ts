...
const stripUnsupportedClassPlanColumns = (payload: Record<string, unknown>) => {
  const {
    ...
    generation_model_version: _ignoredGenerationModelVersion,
    generation_context_snapshot_json: _ignoredGenerationContextSnapshotJson,
    sync_status: _ignoredSyncStatus,
    out_of_sync_reasons_json: _ignoredOutOfSyncReasonsJson,
    manual_overrides_json: _ignoredManualOverridesJson,
    manual_override_mask_json: _ignoredManualOverrideMaskJson,
    last_auto_generated_at: _ignoredLastAutoGeneratedAt,
    last_manual_edited_at: _ignoredLastManualEditedAt,
    weekly_integrated_context_json: _ignoredWeeklyIntegratedContextJson,
    ...legacyPayload
  } = payload;
...
const postClassPlansPayload = async (payload: Record<string, unknown>[]) => {
  try {
...
const parseClassPlanSyncStatus = (
  value: string | null | undefined
): ClassPlan["syncStatus"] => {
  switch (value) {
    case "out_of_sync":
    case "overridden":
    case "stale_parent":
...
const pickString = (...values: (string | null | undefined)[]) =>
  values.find((value) => typeof value === "string" && value.length > 0) ?? "";

const pickNumber = (...values: (number | null | undefined)[]) => {
...
export const mapClassPlanRow = (row: ClassPlanRow): ClassPlan => ({
  ...
  blueprintId: pickString(row.blueprint_id, row.blueprintId),
  generationVersion: pickNumber(row.generation_version, row.generationVersion),
  derivedFromBlueprintVersion: pickNumber(
    row.derived_from_blueprint_version,
    row.derivedFromBlueprintVersion
  ),
  generationModelVersion: pickString(
    row.generation_model_version,
    row.generationModelVersion
  ),
  generationContextSnapshotJson: pickString(
    row.generation_context_snapshot_json,
    row.generationContextSnapshotJson
  ),
  syncStatus: parseClassPlanSyncStatus(row.sync_status ?? row.syncStatus),
  outOfSyncReasonsJson: pickString(row.out_of_sync_reasons_json, row.outOfSyncReasonsJson),
  manualOverridesJson: pickString(row.manual_overrides_json, row.manualOverridesJson),
  manualOverrideMaskJson: pickString(row.manual_override_mask_json, row.manualOverrideMaskJson),
  lastAutoGeneratedAt: pickString(row.last_auto_generated_at, row.lastAutoGeneratedAt),
  lastManualEditedAt: pickString(row.last_manual_edited_at, row.lastManualEditedAt),
  weeklyIntegratedContextJson: pickString(
    row.weekly_integrated_context_json,
    row.weeklyIntegratedContextJson
  ),
  createdAt: row.created_at ?? row.createdat ?? new Date().toISOString(),
  updatedAt:
    row.updated_at ??
...
const classPlanSnapshotPayload = (plan: ClassPlan) => ({
  blueprint_id: plan.blueprintId || null,
  generation_version: plan.generationVersion ?? 1,
  derived_from_blueprint_version: plan.derivedFromBlueprintVersion ?? 1,
  generation_model_version: plan.generationModelVersion ?? "",
  generation_context_snapshot_json: plan.generationContextSnapshotJson ?? "{}",
  sync_status: plan.syncStatus ?? "in_sync",
  out_of_sync_reasons_json: plan.outOfSyncReasonsJson ?? "[]",
  manual_overrides_json: plan.manualOverridesJson ?? "{}",
  manual_override_mask_json: plan.manualOverrideMaskJson ?? "[]",
  last_auto_generated_at: plan.lastAutoGeneratedAt ?? plan.updatedAt,
  last_manual_edited_at: plan.lastManualEditedAt ?? plan.updatedAt,
  weekly_integrated_context_json: plan.weeklyIntegratedContextJson ?? "",
});
...
export async function getClassPlansByClass(
  ...
  try {
    ...
    const mapped = rows.map(mapClassPlanRow);
