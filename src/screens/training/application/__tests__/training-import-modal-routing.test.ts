...
const spreadsheetImportModal = readFileSync(
  resolve(__dirname, "../../components/TrainingSpreadsheetImportModal.tsx"),
  "utf8"
);
...
describe("training import modal routing", () => {
  ...
  it("keeps the legacy URL only as a redirect that opens the modal", () => {
    ...
    expect(spreadsheetImportModal).toContain('presentation="modal"');
  });
});
