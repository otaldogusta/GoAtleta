...
export function AssistantModelSelector({ value, onChange, disabled = false }: {
  value: AssistantModelChoice; onChange: (value: AssistantModelChoice) => void; disabled?: boolean;
}) {
  ...
  return <View style={styles.root}>
    <View ref={anchor} collapsable={false}>
      <Pressable accessibilityRole="button" accessibilityLabel={`Modelo: ${selected.label}`}
        ...
        }} style={[styles.trigger, { opacity: disabled ? 0.55 : 1 }]}>
        <Text numberOfLines={1} style={[styles.title, { color: colors.text }]}>{selected.label}</Text>
