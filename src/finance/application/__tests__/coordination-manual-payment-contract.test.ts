...
const dashboardSource = readFileSync(
  resolve(process.cwd(), "src/screens/finance/CoordinationFinanceDashboard.tsx"),
  "utf8"
);
...
describe("coordination manual payment contract", () => {
  it("collects and confirms the complete manual payment payload", () => {
    ...
    expect(receivablesSource).toContain("Confirmo que este valor já foi recebido");
  });
  ...
  it("shows outstanding balance and names the KPI as open", () => {
    ...
    expect(dashboardSource).not.toContain("outstandingCents || invoice.amountCents");
