...
const STORAGE_KEY = "copilot-fab-position:v1";
const SIZE = 58;
const MARGIN = 16;
type Dock = { side: "left" | "right"; ratio: number };
const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));

export function useDraggableCopilotFab(defaultBottom: number) {
  ...
  const settle = useCallback((x: number, y: number, animate = true) => {
    ...
    void AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(dock.current)).catch(() => undefined);
  }, [position]);
  ...
  return { position, wrapperRef, panHandlers: Platform.OS === "web" ? {} : responder.panHandlers, canOpen: () => Date.now() > suppressClickUntil.current };
}
