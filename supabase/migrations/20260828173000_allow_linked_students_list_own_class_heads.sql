-- Linked athletes need the public identity of the head coach for notification
-- routing. They may resolve only classes where their own athlete record is
-- currently enrolled; organization members keep the existing organization-wide
-- behavior. Private e-mail and source metadata remain server-side.
...
begin
  ...
  v_is_org_member := public.is_org_member(p_org_id);

  if not v_is_org_member and not exists (
    select 1
    from public.students student
    where student.organization_id = p_org_id
      and student.student_user_id = v_user_id
      and coalesce(student.membership_status, 'active') = 'active'
  ) then
    raise exception 'Not authorized';
  end if;
          ...
          and (
            ...
            or exists (
              ...
              where enrollment.organization_id = p_org_id
                ...
                and enrollment.class_id = cs.class_id
