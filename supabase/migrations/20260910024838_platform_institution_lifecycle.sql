-- Platform-owned SaaS lifecycle. It is intentionally independent from athlete
-- tuition/payment messages and never changes organization membership by itself.

create table public.platform_institution_accounts (
  organization_id uuid primary key references public.organizations(id) on delete cascade,
  product text not null default 'goatleta'
    check (product in ('goatleta', 'goatleta_pro')),
  lifecycle_status text not null default 'evaluation'
    check (lifecycle_status in ('evaluation', 'active', 'paused', 'cancelled')),
  commercial_status text not null default 'ok'
    check (commercial_status in ('ok', 'attention')),
  evaluation_ends_at date,
  activated_at date,
  renews_at date,
  responsible_user_id uuid references auth.users(id) on delete set null,
  commercial_note text not null default '' check (char_length(commercial_note) <= 1000),
  updated_at timestamptz not null default now(),
  updated_by uuid references auth.users(id) on delete set null
);

create table public.platform_institution_account_audit (
...
create index platform_institution_accounts_status_idx
  on public.platform_institution_accounts (lifecycle_status, commercial_status, renews_at);
create index platform_institution_account_audit_org_idx
  on public.platform_institution_account_audit (organization_id, created_at desc);

alter table public.platform_institution_accounts enable row level security;
alter table public.platform_institution_account_audit enable row level security;
revoke all on public.platform_institution_accounts from public, anon, authenticated;
revoke all on public.platform_institution_account_audit from public, anon, authenticated;
grant all on public.platform_institution_accounts to service_role;
grant all on public.platform_institution_account_audit to service_role;
...
begin
  ...
  select * into v_before from public.platform_institution_accounts where platform_institution_accounts.organization_id = p_organization_id for update;
...
comment on table public.platform_institution_accounts is
  'GoAtleta SaaS lifecycle per institution. Independent from athlete tuition and never an automatic access gate.';
