create or replace function public.approve_family_registration(p_request_id uuid, p_idempotency_key uuid)
...
begin
  ...
  -- Zero/empty are the existing app convention for an incomplete profile.
  insert into public.students(id,organization_id,name,classid,age,phone,createdat)
    values(sid,r.organization_id,student_name,null,0,'',now());
