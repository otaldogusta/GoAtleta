-- PR7 follow-up: harden date comparison and improve dashboard query indexes

create index if not exists attendance_logs_org_class_date
  on public.attendance_logs(organization_id, classid, date);

create index if not exists session_logs_org_class_createdat
  on public.session_logs(organization_id, classid, createdat);
...
where public.is_org_admin(c.organization_id)
  and not exists (
    ...
    where al.classid = c.id
      and left(al.date::text, 10) = to_char(current_date, 'YYYY-MM-DD')
  );
...
with attendance_actions as (
  ...
  where left(al.date::text, 10) >= to_char(current_date - interval '7 days', 'YYYY-MM-DD')
  group by
    al.organization_id,
    al.classid,
    c.name,
